...
import { RegionConfig, TXSorter } from '@prisma/client'
import { getDailyEntry, saveTXEntry } from '@/app/actions/entry'
import { calculateTX } from '@/lib/calc'
...
export function TXForm({ config, sorters }: { config: RegionConfig, sorters: TXSorter[] }) {
    ...
    useEffect(() => {
        if (!date) return
        setLoading(true)
        getDailyEntry('TX', date).then(entry => {
            if (entry) {
                setPackages(entry.packages)
                setRows(entry.txShifts.map(s => ({ sorterId: s.sorterId.toString(), hours: s.hours })))
    ...
    const calculated = useMemo(() => {
        const shifts = rows.map(r => {
            ...
            return { hours: r.hours, hourlyWage: sorter?.hourlyWage || 0 }
        })
        return calculateTX(packages, shifts, config)
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await saveTXEntry(date, packages, shifts)
                ...
                <CardContent className="pt-6">
                    <div className="grid gap-4">
                        ...
                        <div className="space-y-2">
                             <div className="flex justify-between items-center">
                                <Label>Sorters</Label>
                                <Button size="sm" variant="outline" onClick={handleAddRow}><Plus className="w-4 h-4 mr-2"/> Add Sorter</Button>
                             </div>
                             <div className="rounded-md border">
                             <Table>
                                <TableHeader>
                                    <TableRow>
                                        <TableHead>Sorter Name</TableHead>
                                ...
                                <TableBody>
                                    ...
                                    {rows.length === 0 && (
                                        <TableRow>
                                            <TableCell colSpan={4} className="text-center text-muted-foreground py-6">No sorters added</TableCell>
                                        </TableRow>
                                    )}
                                </TableBody>
                             </Table>
